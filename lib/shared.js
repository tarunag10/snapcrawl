...
const { isFileUrlInsideRoot } = require('./safety');
...
/**
 * Launch a browser with system-Chrome fallback for Chromium.
 *
 * For chromium (the default), tries system Chrome → Edge → bundled Chromium.
 * For firefox/webkit, launches directly (no system equivalents).
 *
 * Config options used:
 *   config.browser        — "chromium" (default), "firefox", "webkit"
 *   config.headless        — true (default) or false
 *   config.browserChannel  — "auto" (default) | "chrome" | "msedge" | "bundled" | any Playwright channel
 */
async function launchBrowser(config) {
  const browserName = config.browser || 'chromium';
  const headless = config.headless !== false;
  const channel = config.browserChannel || 'auto';
  const browserType = browserFromName(browserName);

  // Firefox and WebKit have no system equivalents — launch directly.
  if (browserName === 'firefox' || browserName === 'webkit') {
    console.log(`[browser] Launching ${browserName}`);
    return browserType.launch({ headless });
  }

  // Explicit channel — use it directly (includes "bundled" which means no channel).
  if (channel !== 'auto') {
    if (channel === 'bundled') {
      console.log('[browser] Launching bundled Chromium (forced by config)');
      return browserType.launch({ headless });
    }
    console.log(`[browser] Launching Chromium with channel: ${channel}`);
    return browserType.launch({ headless, channel });
  }

  // Auto mode: try system Chrome → system Edge → bundled Chromium.
  const candidates = [
  ...
  for (const candidate of candidates) {
    try {
      const browser = await browserType.launch({ headless, channel: candidate.channel });
      console.log(`[browser] Using ${candidate.label} (channel: ${candidate.channel})`);
      return browser;
    } catch {
  ...
  // All system browsers failed — fall back to bundled Chromium.
  console.log('[browser] No system Chrome/Edge found — falling back to bundled Chromium');
  return browserType.launch({ headless });
}
...
function normalizeHref(rawHref, currentUrl, rootUrl, options) {
  ...
  if (options.sameOrigin) {
    if (candidate.protocol === 'file:' && rootUrl.protocol === 'file:') {
      if (!isFileUrlInsideRoot(candidate, rootUrl)) return null;
